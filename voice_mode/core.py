...
def get_audio_path(
    filename: str, base_dir: Path, timestamp: Optional[datetime] = None
) -> Path:
...
def get_debug_filename(
    prefix: str, extension: str, conversation_id: Optional[str] = None
) -> str:
...
def get_openai_clients(
    api_key: str, stt_base_url: Optional[str] = None, tts_base_url: Optional[str] = None
) -> dict:
...
async def text_to_speech(
    ...
    logger.info(
        f"TTS: Converting text to speech: '{text[:100]}{'...' if len(text) > 100 else ''}'"
    )
    ...
    try:
        ...
        if clone_profile:
            ...
            logger.info(
                f"  • Voice clone: {clone_profile.name} (ref: {clone_profile.ref_audio})"
            )
        ...
        use_streaming = STREAMING_ENABLED and (
            provider == "cartesia" or validated_format in ["opus", "mp3", "pcm", "wav"]
        )
        ...
        if use_streaming:
            ...
            if success:
                ...
                metrics["playback"] = (
                    stream_metrics.playback_time - stream_metrics.generation_time
                )
                ...
                logger.info(
                    f"✓ TTS streamed successfully - TTFA: {metrics['ttfa']:.3f}s"
                )
        ...
        if provider == "cartesia":
            ...
            async with openai_clients[
                client_key
            ].audio.speech.with_streaming_response.create(**request_params) as response:
                # Read the entire response content
                response_content = await response.read()

        metrics["generation"] = time.perf_counter() - generation_start
        logger.debug(
            f"TTS API response received, content length: {len(response_content)} bytes"
        )
        ...
        with tempfile.NamedTemporaryFile(
            suffix=f".{validated_format}", delete=False
        ) as tmp_file:
            ...
            try:
                ...
                if not loader:
                    ...
                    audio = AudioSegment.from_file(
                        tmp_file.name, format=validated_format
                    )
                else:
                ...
                logger.debug(
                    f"Audio loaded - Duration: {len(audio)}ms, Channels: {audio.channels}, Frame rate: {audio.frame_rate}"
                )
                ...
                if debug:
                    try:
                        ...
                        logger.debug(
                            f"Default output device: {default_output} - {devices[default_output]['name'] if default_output is not None else 'None'}"
                        )
                    except Exception as dev_e:
                        logger.error(f"Error querying audio devices: {dev_e}")

                logger.debug(
                    f"Playing audio with sounddevice at {audio.frame_rate}Hz..."
                )
                ...
                try:
                    ...
                    try:
                        ...
                        if samples.ndim == 1:
                            ...
                            silence = np.zeros(
                                (silence_samples, samples.shape[1]), dtype=np.float32
                            )
                        ...
                        player.play(
                            samples_with_buffer, audio.frame_rate, blocking=False
                        )
                        player.wait()
                        ...
                        if event_logger:
                            tts_event_data = {
                                "metrics": {
                                    ...
                                    "generation_ms": round(
                                        metrics.get("generation", 0) * 1000, 1
                                    ),
                            ...
                            event_logger.log_event(
                                event_logger.TTS_PLAYBACK_END, tts_event_data
                            )
                    ...
                    try:
                        fallback_path = (
                            Path.home()
                            / f"voice-mode-audio-{datetime.now().strftime('%Y%m%d_%H%M%S')}.{validated_format}"
                        )
                        import shutil

                        shutil.copy(tmp_file.name, fallback_path)
                        logger.warning(
                            f"Audio saved to {fallback_path} for manual playback"
                        )
                        os.unlink(tmp_file.name)
                        metrics["playback"] = time.perf_counter() - playback_start
                ...
                logger.error(
                    f"Audio format - Channels: {audio.channels if 'audio' in locals() else 'unknown'}, Frame rate: {audio.frame_rate if 'audio' in locals() else 'unknown'}"
                )
                logger.error(
                    f"Samples shape: {samples.shape if 'samples' in locals() else 'unknown'}"
                )
                ...
                if debug:
                    try:
                        logger.debug(
                            "Attempting alternative playback with system command..."
                        )
                        import subprocess

                        result = subprocess.run(
                            ["paplay", tmp_file.name], capture_output=True, timeout=10
                        )
                        if result.returncode == 0:
                            ...
                            logger.error(
                                f"Alternative playback failed: {result.stderr.decode()}"
                            )
        ...
        logger.error(
            f"TTS config when error occurred - Model: {tts_model}, Voice: {tts_voice}, Base URL: {tts_base_url}"
        )
        ...
        if hasattr(e, "response"):
            logger.error(
                f"HTTP status: {e.response.status_code if hasattr(e.response, 'status_code') else 'unknown'}"
            )
            logger.error(
                f"Response text: {e.response.text if hasattr(e.response, 'text') else 'unknown'}"
            )
            ...
            if hasattr(e.response, "status_code") and e.response.status_code == 401:
                if "openai.com" in tts_base_url:
                    logger.error(
                        "⚠️  Authentication failed with OpenAI. Please set OPENAI_API_KEY environment variable."
                    )
                    logger.error(
                        "   Alternatively, you can use local services (Kokoro) without an API key."
                    )
        elif (
            ...
            if "openai.com" in tts_base_url:
                logger.error(
                    "⚠️  Authentication issue detected. Please check your OPENAI_API_KEY."
                )
                logger.error(
                    "   For local-only usage, ensure Kokoro is running and configured."
                )
        ...
        if (
            hasattr(e, "response")
            or "Error code:" in str(e)
            or isinstance(e, Exception)
            and not error_message.startswith("error playing audio")
        ):
            raise
...
def generate_chime(
    ...
    try:
        ...
        if default_output is not None:
            ...
            if (
                "airpod" in device_name
                or "bluetooth" in device_name
                or "bt" in device_name
            ):
                amplitude = 0.15  # Higher amplitude for Bluetooth devices
                logger.debug(
                    f"Bluetooth device detected ({devices[default_output]['name']}), using amplitude {amplitude}"
                )
            else:
                amplitude = 0.075  # Moderate amplitude for built-in speakers
                logger.debug(
                    f"Built-in speaker detected ({devices[default_output]['name']}), using amplitude {amplitude}"
                )
    except Exception as e:
        logger.debug(
            f"Could not detect output device type: {e}, using default amplitude {amplitude}"
        )
    ...
    actual_leading_silence = (
        leading_silence if leading_silence is not None else CHIME_LEADING_SILENCE
    )
    actual_trailing_silence = (
        trailing_silence if trailing_silence is not None else CHIME_TRAILING_SILENCE
    )
...
async def play_system_audio(
    message_key: str, fallback_text: Optional[str] = None, soundfont: str = "default"
) -> bool:
    ...
    system_audio_dir = (
        Path(__file__).parent / "data" / "soundfonts" / soundfont / "system-messages"
    )
    ...
    if fallback_text:
        logger.info(
            f"Using TTS fallback for system message '{message_key}': {fallback_text}"
        )
