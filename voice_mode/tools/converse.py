...
class DJDucker:
    ...
    def __init__(self, duck_amount: int = None):
        self.duck_amount = (
            duck_amount if duck_amount is not None else DJ_VOLUME_DUCK_AMOUNT
        )
    ...
    def __enter__(self):
        ...
        if self.original_volume is not None:
            ...
            if set_dj_volume(ducked_volume):
                self.ducked = True
                logger.debug(
                    f"DJ ducked: {self.original_volume:.0f}% -> {ducked_volume:.0f}%"
                )
        return self
...
async def startup_initialization():
    ...
    auto_start_kokoro = os.getenv("VOICE_MODE_AUTO_START_KOKORO", "").lower() in (
        "true",
        "1",
        "yes",
        "on",
            ...
            try:
                ...
                if "kokoro" not in service_processes:
                    ...
                    if process.poll() is None:
                        logger.info(
                            f"✓ Kokoro TTS started successfully (PID: {process.pid})"
                        )
                    else:
...
async def get_tts_config(
    ...
    if instructions and model != "gpt-4o-mini-tts":
        logger.warning(
            f"Instructions parameter is only supported with gpt-4o-mini-tts model, ignoring for model: {model}"
        )
        instructions = None
...
async def speech_to_text(
    ...
    file_extension = (
        stt_format if stt_format in ["mp3", "wav", "flac", "m4a", "ogg"] else "mp3"
    )
...
def record_audio_with_silence_detection(
    ...
    logger.info(
        f"record_audio_with_silence_detection called - VAD_AVAILABLE={VAD_AVAILABLE}, DISABLE_SILENCE_DETECTION={DISABLE_SILENCE_DETECTION}, min_duration={min_duration}"
    )

    if not VAD_AVAILABLE:
        logger.warning(
            "webrtcvad not available, falling back to fixed duration recording"
        )
    ...
    try:
        ...
        effective_vad_aggressiveness = (
            vad_aggressiveness if vad_aggressiveness is not None else VAD_AGGRESSIVENESS
        )
        ...
        logger.debug(
            f"VAD config - Aggressiveness: {effective_vad_aggressiveness} (param: {vad_aggressiveness}, default: {VAD_AGGRESSIVENESS}), "
        ...
        if VAD_DEBUG:
            ...
            logger.info(
                f"[VAD_DEBUG]   effective_min_duration: {max(MIN_RECORDING_DURATION, min_duration)}s"
            )
            logger.info(
        ...
        try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if VAD_DEBUG:
                                ...
                                if int(recording_duration * 1000) % 500 == 0:
                                    ...
                                    logger.info(
                                        f"[VAD_DEBUG] t={recording_duration:.1f}s: speech={is_speech}, RMS={rms:.0f}, state={'WAITING' if not speech_detected else 'ACTIVE'}"
                                    )
                        ...
                        if not speech_detected:
                            ...
                            if is_speech:
                                ...
                                if (
                                    VAD_DEBUG and silence_duration_ms % 100 == 0
                                ):  # More frequent logging in debug mode
                                    logger.info(
                                        f"[VAD_DEBUG] Accumulating silence: {silence_duration_ms}/{SILENCE_THRESHOLD_MS}ms, t={recording_duration:.1f}s"
                                    )
                                ...
                                if (
                                    ...
                                    if VAD_DEBUG:
                                        logger.info(
                                            f"[VAD_DEBUG] STOP: silence_duration={silence_duration_ms}ms >= threshold={SILENCE_THRESHOLD_MS}ms"
                                        )
                                        logger.info(
                                    ...
                                    if (
                                        int(recording_duration * 1000) % 500 == 0
                                    ):  # Log every 500ms
                                        logger.info(
            ...
            if chunks:
                ...
                if not speech_detected:
                    ...
                    if VAD_DEBUG:
                        logger.info(
                            f"[VAD_DEBUG] FINAL STATE: No speech was ever detected during recording"
                        )
                else:
                    logger.info(
                    ...
                    if VAD_DEBUG:
                        logger.info(
                            f"[VAD_DEBUG] FINAL STATE: Speech was detected, recording complete"
                        )
...
async def converse(
    ...
    try:
        ...
        try:
            async with audio_operation_lock:
                ...
                if tts_success:
                    try:
                        ...
                        tts_timing_str = (
                            ", ".join(tts_timing_parts) if tts_timing_parts else None
                        )

                        conversation_logger = get_conversation_logger()
                        conversation_logger.log_tts(
                            text=message,
                            audio_file=os.path.basename(tts_metrics.get("audio_path"))
                            if tts_metrics and tts_metrics.get("audio_path")
                            else None,
                            ...
                            if tts_config
                            else (tts_provider if tts_provider else "openai"),
                            provider_url=tts_config.get("base_url")
                            if tts_config
                            else None,
                            provider_type=tts_config.get("provider_type")
                            if tts_config
                            else None,
                            is_fallback=tts_config.get("is_fallback", False)
                            if tts_config
                            else False,
                            fallback_reason=tts_config.get("fallback_reason")
                            if tts_config
                            else None,
                ...
                if not tts_success:
                    ...
                    if (
                        tts_config
                        and tts_config.get("error_type") == "all_providers_failed"
                    ):
                        error_lines = [
                            "Error: Could not speak message. TTS service connection failed:"
                        ]
                        ...
                        for attempt in tts_config.get("attempted_endpoints", []):
                            ...
                            if (
                                attempt.get("error_details")
                                and not openai_error_shown
                                and attempt.get("provider") == "openai"
                            ):
                                error_details = attempt["error_details"]
                                error_lines.append("")
                                error_lines.append(
                                    error_details.get("title", "OpenAI Error")
                                )
                                error_lines.append(error_details.get("message", ""))
                                if error_details.get("suggestion"):
                                    error_lines.append(
                                        f"💡 {error_details['suggestion']}"
                                    )
                                ...
                                endpoint_or_provider = attempt.get(
                                    "endpoint", attempt.get("provider", "unknown")
                                )
                                error_lines.append(
                                    f"  - {endpoint_or_provider}: {attempt['error']}"
                                )
                ...
                logger.debug(
                    f"About to call record_audio_with_silence_detection with duration={listen_duration_max}, disable_silence_detection={disable_silence_detection}, min_duration={listen_duration_min}, vad_aggressiveness={vad_aggressiveness}"
                )
                (
                    audio_data,
                    speech_detected,
                ...
                if event_logger:
                    event_logger.log_event(
                        event_logger.RECORDING_END,
                        {"duration": timings["record"], "samples": len(audio_data)},
                    )
                ...
                logger.info(
                    f"Recording finished at {user_done_time - tts_start:.1f}s from start"
                )
                ...
                if not speech_detected:
                    logger.info(
                        "No speech detected during recording - skipping STT processing"
                    )
                    response_text = None
                    timings["stt"] = 0.0
                    ...
                    if SAVE_AUDIO and AUDIO_DIR:
                        ...
                        audio_path = os.path.join(
                            AUDIO_DIR, f"no_speech_{timestamp}.wav"
                        )
                    ...
                    if isinstance(stt_result, dict):
                        ...
                        if stt_metrics:
                            ...
                            timings["stt_request_ms"] = stt_metrics.get(
                                "request_time_ms", 0
                            )
                            timings["stt_file_size_bytes"] = stt_metrics.get(
                                "file_size_bytes", 0
                            )
                            timings["stt_is_local"] = stt_metrics.get("is_local", False)
                            logger.debug(
                                f"STT metrics: request={stt_metrics.get('request_time_ms')}ms, "
                            ...
                            if stt_result["error_type"] == "connection_failed":
                                ...
                                for attempt in stt_result.get(
                                    ...
                                    if (
                                        ...
                                        if error_details.get("fallback"):
                                            error_lines.append(
                                                f"ℹ️ {error_details['fallback']}"
                                            )
                                        openai_error_shown = True
                                    else:
                                        # Show raw error for non-OpenAI or if we already showed OpenAI error
                                        error_lines.append(
                                            f"  - {attempt['endpoint']}: {attempt['error']}"
                                        )
                ...
                if response_text and should_repeat(response_text):
                    ...
                    if transport == "local":
                        ...
                        audio_path = (
                            tts_metrics.get("audio_path")
                            if "tts_metrics" in locals() and tts_metrics
                            else None
                        )
                        if audio_path and os.path.exists(audio_path):
                            try:
                                ...
                                logger.warning(
                                    f"Failed to replay cached audio: {e}. Regenerating..."
                                )
                                ...
                                if not tts_success:
                                    logger.error(
                                        "Failed to replay audio via TTS regeneration"
                                    )
                        else:
                        ...
                        timings["record"] = (
                            timings.get("record", 0) + record_time
                        )  # Accumulate timing
                        ...
                        if len(audio_data) > 0 and speech_detected:
                            ...
                            timings["stt"] = (
                                timings.get("stt", 0) + stt_time
                            )  # Accumulate timing

                            # Process result
                            if isinstance(stt_result, dict) and not stt_result.get(
                                "error"
                            ):
                                response_text = stt_result.get("text")
                                stt_provider = stt_result.get("provider", "unknown")
                                logger.info(
                                    f"New response after repeat: {response_text}"
                                )

                # Check for wait phrase - if detected, pause for configured duration
                if response_text and should_wait(response_text):
                    logger.info(
                        f"⏸️ Wait requested: '{response_text}'. Pausing for {WAIT_DURATION} seconds..."
                    )

                    # Play system message for wait
                    await play_system_audio(
                        "waiting-1-minute", fallback_text="Waiting one minute"
                    )
                    ...
                    await play_system_audio(
                        "ready-to-listen", fallback_text="Ready to listen"
                    )
                ...
                if event_logger:
                    ...
                    if response_text:
                        event_logger.log_event(
                            event_logger.STT_COMPLETE, stt_event_data
                        )
                    else:
                        event_logger.log_event(
                            event_logger.STT_NO_SPEECH, stt_event_data
                        )
                ...
                try:
                    ...
                    stt_timing_str = (
                        ", ".join(stt_timing_parts) if stt_timing_parts else None
                    )
                    ...
                    conversation_logger.log_stt(
                        ...
                        silence_detection={
                            "enabled": not (
                                DISABLE_SILENCE_DETECTION or disable_silence_detection
                            ),
            ...
            main_timings = {
                k: v for k, v in timings.items() if k in ["tts_total", "record", "stt"]
            }
            ...
            if "stt_file_size_bytes" in timings and timings["stt_file_size_bytes"] > 0:
                stt_timing_parts.append(
                    f"audio {timings['stt_file_size_bytes'] / 1024:.0f}KB"
                )
            ...
            if response_text:
                ...
                if SAVE_TRANSCRIPTIONS:
                    ...
                    save_transcription(
                        conversation_text, prefix="conversation", metadata=metadata
                    )
                ...
                stt_info = (
                    f" (STT: {stt_provider})"
                    if "stt_provider" in locals() and stt_provider != "unknown"
                    else ""
                )
                if effective_metrics_level == "minimal":
                    ...
                    if "stt_request_ms" in timings:
                        verbose_parts.append(
                            f"STT request: {timings['stt_request_ms']:.0f}ms"
                        )
                    if "stt_file_size_bytes" in timings:
                        verbose_parts.append(
                            f"STT file: {timings['stt_file_size_bytes'] / 1024:.0f}KB"
                        )
