        ...
        try:
            ...
            if samples is not None:
                ...
                if (
                    ...
                    and self.audio_queue.qsize() >= self.min_buffer_samples
                ):
        ...
        if self.format == "pcm":
            ...
            try:
                ...
                audio = AudioSegment.from_file(
                    io.BytesIO(data), format=_pydub_format(self.format)
                )
                samples = (
...
async def stream_pcm_audio(
    ...
    try:
            ...
            async for chunk in response.iter_bytes(chunk_size=STREAM_CHUNK_SIZE):
                if chunk:
                    ...
                    if first_chunk_time is None:
                        ...
                        logger.info(
                            f"First audio chunk received after {chunk_receive_time:.3f}s"
                        )
                    ...
                    if debug and chunk_count % 10 == 0:
                        logger.debug(
                            f"Streamed {chunk_count} chunks, {bytes_received} bytes"
                        )
        ...
        if event_logger:
            tts_event_data = {
                "metrics": {
                    "ttfa_ms": round((first_chunk_time - start_time) * 1000, 1)
                    if first_chunk_time
                    else 0,
        ...
        logger.info(
            f"Streaming complete - TTFA: {metrics.ttfa:.3f}s, "
    ...
    try:
            ...
            **request_params
            ...
            async for chunk in response.iter_bytes(chunk_size=STREAM_CHUNK_SIZE):
                if chunk:
                    ...
                    if buffer.tell() > 32768 and not audio_started and format != "opus":
                        buffer.seek(0)
                        try:
                            # Attempt to decode what we have
                            audio = AudioSegment.from_file(
                                buffer, format=_pydub_format(format)
                            )
                            ...
                            samples = (
                                np.array(audio.get_array_of_samples()).astype(
                                    np.float32
                                )
                                / 32768.0
                            )
                            ...
                            audio_started = True
                            logger.info(
                                f"Buffered streaming started - TTFA: {metrics.ttfa:.3f}s"
                            )
        ...
        if buffer.tell() > 0:
            ...
            try:
                ...
                if TTS_TRAILING_SILENCE > 0:
                    pad = np.zeros(
                        int(sample_rate * TTS_TRAILING_SILENCE), dtype=np.float32
                    )
        ...
        if save_audio and save_buffer and audio_dir:
            try:
                ...
                audio_data = save_buffer.read()
                audio_path = save_debug_file(
                    audio_data, "tts", format, audio_dir, True, conversation_id
                )
                if audio_path:
                    logger.info(f"TTS audio saved to: {audio_path}")
                    # Update latest symlinks for quick access to most recent TTS audio
